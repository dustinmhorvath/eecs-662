...
-- CCFAE AST Definition

data CCFAE where
  NumX :: Int -> CCFAE
  PlusX :: CCFAE -> CCFAE -> CCFAE
  MinusX :: CCFAE -> CCFAE -> CCFAE
  MultX :: CCFAE -> CCFAE -> CCFAE
  DivX :: CCFAE -> CCFAE -> CCFAE
  BindX :: String -> CCFAE -> CCFAE -> CCFAE
  LambdaX :: String -> CCFAE -> CCFAE
  AppX :: CCFAE -> CCFAE -> CCFAE
  IdX :: String -> CCFAE
  IfX :: CCFAE -> CCFAE -> CCFAE -> CCFAE
  Closure :: String -> CCFAE -> CEnv -> CCFAE
  deriving (Show,Eq)

type CEnv = [(String,CCFAEVal)]

data CCFAEVal where
  NumV :: Int -> CCFAEVal
  ClosureV :: String -> CCFAE -> CEnv -> CCFAEVal
  deriving (Show,Eq)


evalStatCFBE :: CEnv -> CCFAE -> CCFAEVal
evalStatCFBE cenv (NumX x) = (NumV x)
evalStatCFBE cenv (PlusX l r) = let (NumV l') = (evalStatCFBE cenv l)
                                    (NumV r') = (evalStatCFBE cenv r)
                                in (NumV (l'+r'))
evalStatCFBE cenv (MinusX l r) = let (NumV l') = (evalStatCFBE cenv l)
                                     (NumV r') = (evalStatCFBE cenv r)
                                 in (NumV (l'-r'))
evalStatCFBE cenv (LambdaX i b) = (ClosureV i b cenv)
evalStatCFBE cenv (AppX f a) = let (ClosureV i b e) = (evalStatCFBE cenv f)
                                   a' = (evalStatCFBE cenv a)
                               in evalStatCFBE ((i,a'):e) b
evalStatCFBE cenv (IdX id) = case (lookup id cenv) of
                               Just x -> x
                               Nothing -> error "Varible not found"
evalStatCFBE cenv (IfX c t e) = let (NumV c') = (evalStatCFBE cenv c) in
                                  if c'==0 then (evalStatCFBE cenv t) else (evalStatCFBE cenv e)

-- We're getting values out at the end now
interpStatCCFAE :: String -> CCFAEVal
interpStatCCFAE = (evalStatCFBE []) . parseCCFAE
